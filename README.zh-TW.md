...
#### 案例一：M5Stack 智慧花園（嵌入式韌體）

```
「想做自動澆水的       →  AI 建議 embedded-firmware 管線
  智慧花園」                6 階段、8 個任務、5 人團隊

交付物：
  📄 firmware.cpp (13 KB)  — 完整 ESP32-S3 Arduino 韌體
  📄 platformio.ini         — 建置設定
  📄 pinout.md              — GPIO 腳位規劃 + 感測器校正表
```

#### 案例二：企業 AI 轉型策略（商業辯論）

```
「該不該用 AI 取代       →  AI 建議 quick-prototype 管線
  30% 的員工？」              4 階段辯論結構

辯論過程：
  🔴 自動化派主張全面導入 AI、縮減 30% 人力
  🔵 協作派主張人機協同、零裁員、全員 AI 升級
  ⚖️ CEO 裁定：「人機協同優先」— 全員升級培訓、僅自動化已驗證安全的流程、
      先啟動 3 個內部試點專案

交付物：
  📄 debate.md (8 KB) — 四維度結構化辯論 + 18 個月實施路徑圖
```
