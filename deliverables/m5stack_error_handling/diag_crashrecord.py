#!/usr/bin/env python3
"""
Diagnose the CrashRecord checksum issue by creating a standalone C++ program.
"""
...
with open("diag.cpp", "w") as f:
    f.write(diag_src)

r = subprocess.run(["g++", "-std=c++20", "-I.", "-o", "diag", "diag.cpp"],
                   capture_output=True, text=True)
if r.returncode != 0:
    print("COMPILE ERROR:", r.stderr, r.stdout)
    sys.exit(1)

r = subprocess.run(["./diag"], capture_output=True, text=True)
print(r.stdout)
if r.stderr:
